#!/usr/bin/env python3
"""Pre-tool-use hook to enforce DAIC (Discussion, Alignment, Implementation, Check) workflow and branch consistency - adapted for Cursor."""
import json
import sys
import subprocess
...
# Load configuration from project's .cursor directory
...
# Check if we're in a subagent context and trying to edit .cursor/state files
project_root = get_project_root()
...
if subagent_flag.exists() and tool_name in ["Write", "Edit", "MultiEdit"]:
    ...
    if file_path_str:
        ...
        try:
            ...
            # If we get here, the file is under .cursor/state
            print(f"[Subagent Boundary Violation] Subagents are NOT allowed to modify .cursor/state files.", file=sys.stderr)
            print(f"Stay in your lane: You should only edit task-specific files, not system state.", file=sys.stderr)
            sys.exit(2)  # Block with feedback
        except ValueError:
            # Not under .cursor/state, which is fine
            pass
        ...
        if expected_branch:
            ...
            if repo_path:
                try:
                    ...
                    # Get project root (parent of .cursor directory)
