#!/usr/bin/env python3
"""User message hook to detect DAIC trigger phrases and special patterns - adapted for Cursor."""
import json
import sys
import re
import os
...
if transcript_path and tiktoken and os.path.exists(transcript_path):
    ...
    if context_length > 0:
        ...
        if usable_percentage >= 90 and not warning_90_flag.exists():
            context += f"\n[90% WARNING] {context_length:,}/160,000 tokens used ({usable_percentage:.1f}%). CRITICAL: Use /complete-task command to wrap up this task cleanly!\n"
...
if any(phrase in prompt_lower for phrase in ["compact", "restart session", "context compaction"]):
    context += "If the user is asking to compact context, use the /compact command or restart the session.\n"
...
if any(phrase in prompt_lower for phrase in ["complete the task", "finish the task", "task is done",
    ...
    context += "If the user is asking to complete the task, use the /complete-task command.\n"

# Task creation detection
if any(phrase in prompt_lower for phrase in ["create a new task", "create a task", "make a task",
                                               "new task for", "add a task"]):
    context += "If the user is asking to create a task, use the /create-task command.\n"

# Task switching detection
if any(phrase in prompt_lower for phrase in ["switch to task", "work on task", "change to task"]):
    context += "If the user is asking to switch tasks, use the /start-task command.\n"
...
If they want to create a task, use the /create-task command.
"""
