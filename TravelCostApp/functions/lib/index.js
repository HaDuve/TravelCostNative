...
const DISCORD_WEBHOOK_URL = ((_a = functions.config().discord) === null || _a === void 0 ? void 0 : _a.webhook_url) || process.env.DISCORD_WEBHOOK_URL;
/**
        ...
        embeds: [
            {
                ...
                fields: [
                    ...
                    {
                        name: "💬 Feedback",
                        value: feedback.feedbackString.length > 1000
...
async function sendDiscordNotification(message) {
    if (!DISCORD_WEBHOOK_URL) {
        throw new Error("Discord webhook URL not configured. Please set DISCORD_WEBHOOK_URL");
    }
    ...
    if (response.status < 200 || response.status >= 300) {
        throw new Error(`Discord webhook returned status ${response.status}: ${JSON.stringify(response.data)}`);
    }
}
