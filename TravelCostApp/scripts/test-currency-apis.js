...
async function testAPI1(base, target) {
  ...
  try {
    ...
    console.log(`📊 Response data structure:`, {
      ...
      ratesCount: response.data.rates
    ...
    if (response.data.rates && response.data.rates[target]) {
      console.log(
        `✅ Found rate for ${target}: ${response.data.rates[target]}`
      );
      return {
        success: true,
...
async function runTests() {
  ...
  for (const pair of testPairs) {
    ...
    console.log(
      `   API1: ${api1Result.success ? "✅" : "❌"} ${api1Result.rate || "N/A"}`
    );
    console.log(
      `   API2: ${api2Result.success ? "✅" : "❌"} ${api2Result.rate || "N/A"}`
    );
  }
